...
class Clients:
    ...
    def build_url(self, id=None):
        if not id:
            return f"{os.getenv('PETCARE_API_URL')}clients/?auth={os.getenv('PETCARE_AUTH_TOKEN')}"
        elif id:
            return f"{os.getenv('PETCARE_API_URL')}clients/{id}?auth={os.getenv('PETCARE_AUTH_TOKEN')}"
